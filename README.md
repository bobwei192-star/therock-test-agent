...
本项目支持在 **native Linux** 和 **WSL/WSL2** 中运行。native Linux 使用 ROCm 常规设备模型（`/dev/kfd`、`/dev/dri`）；WSL2 使用 ROCm on WSL 设备模型（`/dev/dxg`），此时 `/dev/kfd` 和 `/dev/dri` 缺失是正常现象。runner 会自动识别运行环境并把 `runtime_label` 写入报告。
...
| 环境 | 支持状态 | GPU 设备模型 | 说明 |
|------|----------|--------------|------|
| native Linux | 支持 | `/dev/kfd` + `/dev/dri` | 推荐用于对齐 TheRock 上游 Linux 组件测试。 |
| WSL/WSL2 | 支持运行 agent；GPU 测试以 WSL2 为准 | `/dev/dxg` | 需要 Windows 主机、WSL2、ROCm on WSL 和 `/dev/dxg` 正常可见。 |

注意：TheRock 上游文档中的 WSL 主要用于 CI 的 `wsl-rocdxg` 构建阶段；本地从源码构建和完整组件测试仍以 native Linux / native Windows 为主。若在 WSL2 中使用本项目运行测试，请准备好可用的 ROCm artifacts，并先确认 `/dev/dxg` 存在。
...
- **native Linux / WSL2 环境识别**：自动区分 `linux-rocm`、`wsl2-dxg`、`wsl2-missing-dxg` 等运行标签，并按 WSL2 的 `/dev/dxg` 模型做 preflight。
...
在 WSL2 中运行 agent 时，建议优先使用已经准备好的 ROCm artifacts 或在 native Linux 上构建后的产物；TheRock 上游当前并不把本地 WSL2 源码构建作为主要支持路径。
...
| `debug_repair=<opencode\|off>` | 可选。默认 `opencode`：每轮失败后暂停，等 OpenCode debug/repair 后再 resume。`off` 则 runner 自动重跑失败集，不等待分析。 |

### 全自动 debug → repair → 下一轮

**现在能不能做到？分两种启动方式：**

| 启动方式 | 测完一轮后 | debug | repair | 自动下一轮 |
|----------|------------|-------|--------|------------|
| OpenCode `/therock-run`（`therock-loop` 编排） | `waiting_for_opencode_debug` | ✅ 应自动调用 `/therock-debug-round` | ✅ 应自动调用 `/therock-repair-round apply=safe` | ✅ 应自动 `/therock-resume` |
| 直接 `start-kv` / `run-kv`（纯 runner CLI） | `waiting_for_opencode_debug` | ❌ 不自动 | ❌ 不自动 | ❌ 需手工 `resume` |

结论：

- **要「失败后自动 debug → repair → 下一轮」**，请用 OpenCode 的 `/therock-run`，并让整个会话保持运行直到 `passed` / `failed` / `max_rounds`。`therock-run.md` 和 `therock-loop.md` 已写明编排规则；WSL2 run `20260717_133238_7a7b6bfe` 就是这套流程跑通的例子。
- **若只执行 `start-kv` 就离开**，runner 会在每轮失败后停下，**不会**自动调 OpenCode debugger/repairer。native Linux run `20260710_190554_105b10f6` 若只 `resume` 而不先 debug，就会出现 issue6 里的困惑。
- **runner 里尚未实现 `debug_repair=auto`**（见 `docs_this_project/改进文档3.md`）。当前 CLI 只支持 `opencode`（暂停等 OpenCode）和 `off`（跳过等待、直接 loop）。要在**无 OpenCode 会话**时也全自动，需要新增编排层（见下文「无头模式」）。

`/therock-run` 每轮失败后的标准编排（OpenCode 内自动执行，也可手工逐步跑）：
...
验证是否走完 debug：检查 `progress.jsonl` 是否有 `opencode_debug_written`，以及 `round_analysis/round<N>.json` 是否存在。

若要**暂时跳过 debug 等待、只重跑失败集**（旧行为）：

```text
/therock-run ... debug_repair=off
```

### 长测：用后台 start，不要用会超时的前台 bash

全量 / full / rocblas 等单任务可达 1–3 小时。**不要**在 OpenCode 前台 shell 里直接 `run-kv`（易被会话超时杀掉，状态卡在 `running` 但进程已死，见 issue6）。
...
# 1. 后台启动（返回 run_id 后立即退出，不受前台超时影响）
.opencode/tools/therock_agent.sh start-kv \
  ...
  test_types=quick \
  gpu_risk=skip \
  debug_repair=opencode

# 2. 查进度（可反复执行）
.opencode/tools/therock_agent.sh status <run_id>

# 3. 跟踪事件流
tail -f runs/<run_id>/progress.jsonl
```

在 OpenCode 里等价于 `/therock-run ...`（内部也是 `start-kv` + 编排）。长测时 OpenCode 侧应**轮询 status**，等 `waiting_for_opencode_debug` 后再接 debug/repair/resume，而不是在前台阻塞等单个 task 结束。
...
无 GUI、SSH 服务器或希望 OpenCode/Cursor 后台持续编排时：

1. **测试执行**：始终用 `start-kv` 放后台（上一节）。
2. **debug/repair 编排**：仍依赖 OpenCode agent（`therock-loop` → `therock-debugger` / `therock-repairer`），不是 runner 内置功能。
3. **CodeGraph MCP**（可选）：在 `.cursor/mcp.json` 配置本地 `codegraph` 服务，供 Cursor/OpenCode 无头读取项目上下文；路径请改成本机 checkout：
...
`.codegraph/` 目录仅存放本机 CodeGraph 数据库/套接字（已在 `.gitignore` 忽略），不提交到 git。

无头推荐启动方式（二选一）：

```bash
# A. OpenCode + sudo askpass（需要交互一次密码）
cd ~/TheRock
./scripts/therock-sudo-agent run -- opencode
# 会话内：/therock-run artifacts=... gpu=gfx1151 components=all test_types=quick gpu_risk=skip
```

```bash
# B. 纯 runner 后台 + 另开会话做编排（适合 cron/SSH）
cd ~/TheRock
.opencode/tools/therock_agent.sh start-kv artifacts=... gpu=gfx1151 components=all test_types=quick
# 等 waiting_for_opencode_debug 后，用 OpenCode/Cursor 执行 debug-round → repair-round → resume
```

**若要做到完全无人值守的 debug→repair→下一轮**，还需实现（见 `docs_this_project/改进文档3.md`）：

- runner 新增 `debug_repair=auto`（或独立 `scripts/therock-orchestrator.sh` 轮询 `status` 并调用 OpenCode CLI/API）；
- 将 `therock-debugger` / `therock-repairer` 的落盘逻辑抽成可脚本化入口（当前设计刻意放在 OpenCode 层）。
...
若已用 `/therock-run` 启动且 OpenCode 会话仍在，**一般不需要**手工执行下面两条；仅当编排中断、或直接用 `start-kv` 启动时才需要：
...
直接后台调用 runner（**只跑测试，不自动 debug/repair**；长测请用此方式，不要用前台 `run-kv`）：
...
### WSL2 下任务被 `missing_wsl_dxg` 阻断

WSL2 的 ROCm GPU 访问依赖 `/dev/dxg`。先确认：
...
```text
/home/<user>/TheRock/output-linux-portable/build
```

避免把大量测试日志和 artifacts 放在 `/mnt/c/...` 这类 Windows 挂载路径上，以减少 I/O 性能和路径兼容问题。
