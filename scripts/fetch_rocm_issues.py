...
def handle_rate_limit(response: requests.Response, headers: dict):
    ...
    if response.status_code in (403, 429):
        ...
        print(f"  ⏳ 触发速率限制，等待 {wait} 秒 (到 {datetime.fromtimestamp(reset_ts).strftime('%H:%M:%S')})...")
        time.sleep(wait)
        return True
    return False


def fetch_with_retry(url: str, headers: dict, params: dict | None = None) -> requests.Response:
    """带重试的 GET 请求。"""
...
def format_issue_body(issue: dict, comments: list[dict]) -> str:
    ...
    lines.append(f"> **作者**: {issue['user']['login'] if issue.get('user') else 'unknown'}")
...
def main():
    ...
    readme_lines.extend(["", "## 文件结构", "", "- `index.json` - 所有 Issue 的汇总索引"])
