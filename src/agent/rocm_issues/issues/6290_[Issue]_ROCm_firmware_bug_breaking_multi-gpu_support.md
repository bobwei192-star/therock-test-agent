...
> **Issue #6290**
> **状态**: open
> **创建时间**: 2026-05-21T18:08:16Z
> **更新时间**: 2026-05-26T15:08:09Z
> **作者**: AERProductions
> **标签**: status: triage
...
### 评论 #1 — Kinetic-Labs-GT (2026-05-22T08:00:58Z)

Based on the provided terminal traces, hardware topology, and `rocminfo` data, this issue cannot be resolved via a user-space code patch within this repository. The root cause is a binary microcode regression within the closed-source **MES (MicroEngine Scheduler)** firmware component, distributed upstream via the `linux-firmware-amdgpu` package.

### Technical Deconstruction

1. **The Core Fault:** The memory access fault (`GCVM_L2_PROTECTION_FAULT_STATUS`) on GPU Node-1 flags a critical memory protection violation during Peer-to-Peer (P2P) buffer initialization across the PCIe bus.
2. **Topology Aggravation:** The `rocminfo` output indicates an active integrated GPU (`gfx1036` / Node 3) running concurrently with dual discrete `gfx1201` cards. The highly asymmetrical physical layout—where GPU 0 operates on a CPU-attached PCIe 5.0 x16 link and GPU 1 operates on a chipset-multiplexed PCIe 4.0 x4 link—triggers a boundary condition in how the MES microcode handles physical-to-virtual memory mapping page tables (VRAM vs. GTT) during multi-GPU tensor splitting.
3. **Upstream Isolation:** Because the application layer is executing valid HIP API calls and the `ROCR-Runtime` user-space libraries are correctly passing these calls to the kernel layer, modifying user-space source code cannot bypass or fix this low-level hardware scheduler panic.

---

### Actionable Diagnostic Workarounds

While an architectural fix must come from the AMD kernel firmware team, you can deploy the following workarounds to isolate or mitigate the fault:

* **Disable the Integrated GPU (iGPU):** Access your motherboard UEFI/BIOS settings and completely disable the CPU's integrated graphics. This removes Node 3 from the HSA topology map, eliminating unnecessary memory mapping complexity across the asymmetric PCIe lanes.
* **Rollback Linux Firmware:** Temporarily downgrade the `linux-firmware` (or `linux-firmware-amdgpu`) package on your host OS to a stable release snapshot deployed prior to the current driver cycle.
* **Upstream Ticket Tracking:** Because this is a kernel/firmware isolation fault, this tracking matrix belongs on the [Freedesktop AMDGPU GitLab Issues Tracker](https://gitlab.freedesktop.org/drm/amd/-/issues), where upstream kernel driver and firmware maintainers triage hardware execution bugs.

---

### 评论 #2 — JoshuaBowerman (2026-05-26T15:08:09Z)

I'll leave this here in case it helps anyone with similar issues. This problem doesn't occur with kernels <=6.17.12 so downgrading your kernel might help temporarily.

https://gitlab.freedesktop.org/drm/amd/-/work_items/5256
