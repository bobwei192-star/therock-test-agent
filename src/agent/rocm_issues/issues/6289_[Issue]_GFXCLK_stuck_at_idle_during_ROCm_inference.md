...
> **Issue #6289**
> **状态**: open
> **创建时间**: 2026-05-21T15:15:47Z
> **更新时间**: 2026-05-25T17:39:51Z
> **作者**: vincentlaloux
> **标签**: status: triage
...
### 评论 #1 — Kinetic-Labs-GT (2026-05-22T08:03:19Z)

Based on your exhaustive isolation and diagnostic data, this issue cannot be resolved via an open-source pull request within this user-space runtime repository. The problem is deep-seated within the **SMU14 (System Management Unit)** power-management microcode and the amdgpu kernel driver's frequency-scaling governor tracking logic.

### Technical Deconstruction

1. **The Core Mechanism:** Standard graphics workloads submit commands through traditional ring buffers, which inherently trigger the driver's frequency scaling governor to scale up DPM clock states. Conversely, ROCm/HSA compute queues rely on direct user-space queue submissions via **doorbells**, bypassing the kernel for low-latency dispatch. On this RDNA4 hardware cycle, the SMU14 firmware fails to register these asynchronous KFD memory ring transactions as a valid compute power state transition, locking the core clock at its deepest sleep state (41 MHz).
2. **The gpu_busy_percent vs. mem_busy_percent Anomaly:** The reason gpu_busy_percent reads a false 100% when profile_peak is enforced is that disabling clock gating forces the Shader Engines into an always-on, high-leakage state. The kernel driver misinterprets this lack of power-gating as 100% execution utilization. Your pivot to tracking mem_busy_percent (VRAM memory controller activity) is the mathematically correct signal for identifying active inference memory passes.
3. **Topology Layout:** The required use of HSA_OVERRIDE_GFX_VERSION=12.0.1 and HIP_VISIBLE_DEVICES=0 confirms that the user-space layer is behaving deterministically. Masking the CPU’s integrated display engine (gfx1036) prevents the runtime from allocating execution graphs or context frames to system memory pools, which would otherwise result in catastrophic performance degradation.

---

### Workaround Assessment

The provided amdgpu-dynperf.py systemd daemon script is a well-engineered stopgap. Polling the sysfs node for memory controller utilization to programmatically flip the global performance level via power_dpm_force_performance_level is the only viable method to bypass the broken internal SMU firmware scaling until a structural fix is mainlined.

### Upstream Tracking

Because this requires an update to the closed-source SMU firmware blob packaged in linux-firmware along with corresponding updates to the amdgpu kernel driver's PM core, this issue must be tracked at the source. It is highly recommended to port this exact log matrix directly to the [[Freedesktop AMDGPU GitLab Issues Tracker](https://gitlab.freedesktop.org/drm/amd/-/issues)](https://gitlab.freedesktop.org/drm/amd/-/issues) for triage by the kernel engineering team.
