...
> **Issue #582**
> **状态**: closed
> **创建时间**: 2018-10-23T02:20:00Z
> **更新时间**: 2018-11-06T17:46:55Z
> **关闭时间**: 2018-11-06T17:46:55Z
> **作者**: leimingyu
> **标签**: 
...
### 评论 #1 — jlgreathouse (2018-10-23T15:53:20Z)
...
This listing implies a few things for me:

- It appears that you have amdgpu-pro installed (this is `amdgpu 17.40-492261`). ROCm and amdgpu-pro shouldn't be used together, so I recommend [uninstalling that](https://support.amd.com/en-us/kb-articles/Pages/AMDGPU-PRO-Install.aspx) (e.g. with `amdgpu-pro-uninstall`).
- You are using ROCm 1.9.0, while we recently released 1.9.1. The most up-to-date ROCm will install `amdgpu 1.9-224`. This is not extremely important, since ROCm 1.9.1 was a point release with only a few small changes. But running `apt update; apt upgrade` should install ROCm 1.9.1 for you, and should properly build the driver for your new kernel.
- The real underlying issue is that you don't have the proper version of the ROCm drivers built for your kernel. Note that in your output to `dkms status` you only have the ROCm version of amdgpu (1.9-x) installed for kernel versions `4.15.0-33-generic` and `4.15.0-34-generic`. However, you're running `4.15.0-36-generic`. As such, the ROCm-supplied drivers are not being loaded. Instead, you're using the versions that come with the 4.15 kernel, which are not fully compatible with our user-land code.

To fix the third issue, you could either upgrade to ROCm 1.9.1 (which should install the ROCm driver for your current kernel), or you can run the following series of commands to rebuild your existing driver for your current kernel:
```
sudo dkms build amdgpu/1.9-211
sudo dkms install amdgpu/1.9-211
```
...
```
sudo dkms build amdgpu/1.9-211 -k {kernel_version}
sudo dkms install amdgpu/1.9-211 -k {kernel_version}
```
