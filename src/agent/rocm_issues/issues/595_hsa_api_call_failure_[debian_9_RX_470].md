...
> **Issue #595**
> **状态**: closed
> **创建时间**: 2018-10-31T11:08:51Z
> **更新时间**: 2018-11-06T17:43:42Z
> **关闭时间**: 2018-11-06T17:43:42Z
> **作者**: traks86
> **标签**: 
...
### 评论 #1 — jlgreathouse (2018-10-31T14:03:46Z)

What model CPU are you using? Based on the PCI IDs, I suspect it is an [Ivy Bridge Xeon E3-12xx v2](http://pci-ids.ucw.cz/read/PC/8086/0150) model. As far as I know (and looking at the output of your lspci dump), these CPUs do not support PCIe 3.0 atomics, which are [required in order to use your gfx8 GPU in the ROCm stack](https://github.com/RadeonOpenCompute/ROCm#supported-cpus).

To verify, please run `dmesg | grep kfd`. I would wager it will say `kfd kfd: skipped device 1002:67df, PCI rejects atomics` which means the kfd driver cannot enable your GPU for use by the user-level ROCm software.

---

### 评论 #2 — traks86 (2018-10-31T14:47:08Z)

I'm using an Ivy Bridge i5-3570K on a Z77 chipset (GA-Z77-DS3H) which support pcie 3.0. 
However my main pcie-16x port is dead so I've connected my GPU to the second pcie-16x port which run at x4 and pcie 2.0 I believe.

But I think there is hope : 
```
$ dmesg | grep kfd
[    7.726326] kfd kfd: Initialized module


---

### 评论 #3 — jlgreathouse (2018-10-31T15:27:33Z)

While your CPU may support PCIe 3.0, that does not mean those slots support the PCIe-optional atomics features that we require for ROCm on gfx8 GPUs. In addition, if you're connected through a PCIe 2.0 port (or through one of the chipset-connected PCIe ports), you're also out of luck.

If the kfd isn't even running a check against your GPU, then the drivers likely aren't installed correctly either. What do you see when you do `dkms status`?

That said, based on what you've told me about your hardware, I strongly suspect that even if we fix your driver problems, your GPU will not work in the ROCm stack.

---

### 评论 #4 — traks86 (2018-10-31T15:58:28Z)

Indeed I think I'm out of luck, MB manual says it's a chipset 2.0 pci-e port.
```
dkms status
amdgpu, 1.9-244: added
```
How do you see that kfd isn't running and my driver is not correctly installed ?


---

### 评论 #5 — jlgreathouse (2018-10-31T16:12:26Z)

The driver isn't installed correctly, because the up-to-date version of the `amdkfd` will print out extra messages whenever it runs across an AMD GPU. The fact that it only initialized the module but gave no information about either initializing (or failing to initialize because of lack of PCI atomics) your GPU means that this is some older upstream kfd.

If you have a working ROCK installation, you'll see something like:
```
$ dkms status
amdgpu, 1.9-224, 4.9.0-8-amd64, x86_64: installed
```

You're only at the "added" stage. The next stage is "built", then "installed". So it sounds like the build amdkfd as part of `rock-dkms` failed.

If you run `dkms build amdgpu/1.9-224`, what happens? (I'm assuming it will fail and point you towards a log showing the compilation problem).

---

### 评论 #6 — traks86 (2018-10-31T16:23:54Z)

Indeed, it crashs
```
In file included from /var/lib/dkms/amdgpu/1.9-224/build/amd/amdgpu/../backport/backport.h:8:0,
                 from <command-line>:0:
/var/lib/dkms/amdgpu/1.9-224/build/include/kcl/kcl_fence.h: In function ‘kcl_dma_fence_set_error’:
/var/lib/dkms/amdgpu/1.9-224/build/include/kcl/kcl_fence.h:163:7: error: ‘struct fence’ has no member named ‘status’
  fence->status = error;
```
So there's no hope due to the lack of pcie 3.0, isn't it ?

Anyway, thank you for your help. :)

---

### 评论 #7 — jlgreathouse (2018-10-31T16:53:51Z)

Well, it looks like we have some patches we would need to put into place to compile on your Debian release. That's a separate issue. But yes, even if we got your driver compilation working, your gfx8 GPU would not work with ROCm. gfx9 GPUs (such as our Vega-class GPUs like a Radeon RX Vega 56) would work even without atomics. But your gfx8 GPU need them.
