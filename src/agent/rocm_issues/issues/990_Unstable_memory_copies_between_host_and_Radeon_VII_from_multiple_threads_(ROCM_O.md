...
> **Issue #990**
> **状态**: closed
> **创建时间**: 2020-01-02T14:31:50Z
> **更新时间**: 2020-01-09T15:42:45Z
> **关闭时间**: 2020-01-09T15:40:35Z
> **作者**: drtpotter
> **标签**: 
...
### 评论 #1 — gandryey (2020-01-07T23:41:25Z)

Hello,
Could you switch to single OCL context and prepin **array** (using clCreateBuffer() with CL_MEM_USE_HOST_PTR)  before calling the **worker** function? That should avoid the crash and improve performance. 

---

### 评论 #2 — drtpotter (2020-01-08T12:36:04Z)

Hi there,

Cheers for this comment, if my hardware is ok then it appears that the example above has revealed some threadsafety shortcomings with the ROCM OpenCL implementation on Radeon VII. If these were addressed it would certainly improve the library, which is what we all want. By reducing the number of threads to the number of OpenCL devices (1 thread and 1 context per device) I was able to avoid the crash, but performance was a lot slower than with other OpenCL implementations. Here is the output from the run with 3 threads on 3 OpenCL devices (POCL, CUDA OpenCL, ROCM).

```bash
Number of devices found = 3
Device pthread-AMD Ryzen Threadripper 2950X 16-Core Processor finished 342 slices in 1.89098 seconds.
Device GeForce GTX 1060 6GB finished 341 slices in 0.955199 seconds.
Device gfx906+sram-ecc finished 341 slices in 56.9739 seconds.
```

It's a pretty serious thing for a userland OpenCL application to force a system reboot as occasionally seen with this example and a larger number of threads. If the ROCM OpenCL implementation on Radeon VII is not thread safe then it should at least fail in a safe manner. I'll give pre-pinning a try.


---

### 评论 #3 — drtpotter (2020-01-08T16:17:06Z)

As an update on this I modified the implementation to always use 1 context per device and to pre-pin the host memory with CL_MEM_ALLOC_HOST_PTR and mapping it before passing to the threads. With 1 context and 1 thread per device mem copies are still much slower on the Radeon VII than on the other devices.

```bash
Number of devices found = 3
Device GeForce GTX 1060 6GB finished 170 slices in 0.122918 seconds.
Device pthread-AMD Ryzen Threadripper 2950X 16-Core Processor finished 172 slices in 0.421796 seconds.
Device gfx906+sram-ecc finished 170 slices in 7.19322 seconds.
```
With 1 context and 2 command queues per device (6 threads total) it forced my system to reboot. Here is the code that produced the crash.
...
void worker(int64_t rank, int64_t nworkers, float_type* host_ptr, 
        cl_context compute_context, cl_device_id compute_device) {
...
int main() {
    ...
    for (cl_uint i=0; i<numPlatforms; i++) {
        ...
        if (errcode==CL_SUCCESS) {
            ...
            for (cl_uint n=0; n<ndevices; n++) {
                devices.push_back(temp_devices[n]);
                temp_platforms.push_back(platforms[i]);
                const cl_context_properties prop[] = { CL_CONTEXT_PLATFORM, (cl_context_properties)platforms[i], 0 };
                cl_context temp_context=clCreateContext(prop, 1, &temp_devices[n], NULL, NULL, &errcode);
                contexts.push_back(temp_context);
                errchk(errcode,"creating a context");
            }
            delete [] temp_devices;
        }    
    ...
    // Create a master command queue for mapping etc...
    cl_command_queue master_queue=clCreateCommandQueue(contexts[0], devices[0], 0, &errcode);
    errchk(errcode,"creating a master command queue");

    // Allocate memory for array
    cl_mem array_buffer = clCreateBuffer(contexts[0], CL_MEM_ALLOC_HOST_PTR, NX*NY*NZ*sizeof(float_type), NULL, &errcode );
    errchk(errcode,"creating a master buffer");
    float_type* array = (float_type*)clEnqueueMapBuffer(master_queue, array_buffer, CL_TRUE, 
        CL_MAP_READ | CL_MAP_WRITE, 0, NX*NY*NZ*sizeof(float_type), 0, NULL, NULL, &errcode);
    errchk(errcode,"mapping the master buffer");
    ...
    for (int64_t t=0; t<NTHREADS; t++) {
        cl_uint index = t%devices.size();
        worker_threads[t]=std::thread(worker, t, (int64_t)NTHREADS, array, contexts[index], devices[index]);
    }
    ...
    for (int n=0; n<contexts.size(); n++) {
        errchk(clReleaseContext(contexts[n]), "Releasing the context");
    }
    
    // Unmap memory
    clEnqueueUnmapMemObject(master_queue, array_buffer, array, 0, NULL, NULL);
    errchk(clReleaseMemObject(array_buffer), "Releasing the main memory buffer");
    errchk(clReleaseCommandQueue(master_queue), "Releasing master_queue");
}
```

---

### 评论 #4 — gandryey (2020-01-08T23:59:49Z)

Unfortunately I can't reproduce the reported issues with the latest(prepin) logic, but I use Ubuntu and have more recent base driver. I would suggest to check 19.50 base driver. Also clinfo dump can be more helpful to identify the exact config on your system.

---

### 评论 #5 — drtpotter (2020-01-09T15:40:34Z)

I found the cause of the issue, upgrading to the 19.50 driver didn't help, nor did downgrading my kernel to the operating system default (4.12). Reinstalling CUDA for my co-installed GTX 1060 without the nvidia-drm kernel module caused everything to start working properly with the Radeon VII and this example. So it appears the problem wasn't with ROCM but interference from NVIDIA's nvidia-drm kernel driver?!
