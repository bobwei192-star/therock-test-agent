...
> **Issue #758**
> **状态**: closed
> **创建时间**: 2019-04-09T16:46:19Z
> **更新时间**: 2023-12-11T17:02:57Z
> **关闭时间**: 2023-12-11T14:55:00Z
> **作者**: syoyo
> **标签**: 
...
### 评论 #1 — jamilbk (2019-05-14T19:53:16Z)

I'm getting the same issue. Simply launching `clinfo` or any program that tries to read more than the first GPU results in an amdgpu crash (`Page not present or...`)

Ubuntu 18.04
Tried Kernels 4.15, 4.18, 5.0
ROCm 2.3, 2.4
Gigabyte Aorus Gaming 7 X399 (BIOS F12e)
Four Vega FEs

Disabling IOMMU in the BIOS seems to fix the issue, but I'd rather not have to resort to this.


---

### 评论 #2 — syoyo (2019-05-15T03:47:30Z)

@jamilbk Thanks you for the report. 

I think IOMMU is optional for dGPUs configuration, thus we can disable IOMMU as a work around... > https://github.com/RadeonOpenCompute/ROCK-Kernel-Driver/issues/25

FYI, I have tested x299 + 2 VEGAs and it works well, thus the issue would be specific to x399.

---

### 评论 #3 — seesturm (2019-05-15T05:53:50Z)

Cannot really help, but only came here to report that 2x Vega works on my x399 platform with IOMMU enabled. So there is no fundamental problem with IOMMU + Vega + x399.

Ubuntu 18.04
Kernel 5.1 (no rock-dkms installed, probably also works with 5.0 and 4.20)
ROCm 2.4, 2.3 (probably works also with older)
TR 1950x
64GB RAM
MSI X399 SLI PLUS (Bios 7B09vA7, above 4G decoding and NUMA enabled)
2x Vega 10 XT [Radeon RX Vega 56]
```
# lspci -v
0c:00.0 VGA compatible controller: Advanced Micro Devices, Inc. [AMD/ATI] Vega 10 XT [Radeon RX Vega 64] (rev c3) (prog-if 00 [VGA controller])
        ...
        Memory at e1d00000 (32-bit, non-prefetchable) [size=512K]
        Expansion ROM at e1d80000 [disabled] [size=128K]
 ...
44:00.0 VGA compatible controller: Advanced Micro Devices, Inc. [AMD/ATI] Vega 10 XT [Radeon RX Vega 64] (rev c3) (prog-if 00 [VGA controller])
        Subsystem: Advanced Micro Devices, Inc. [AMD/ATI] Vega 10 XT [Radeon RX Vega 64]
        Flags: bus master, fast devsel, latency 0, IRQ 92, NUMA node 1
...
```
I'm sure that my IOMMU is active since I suffer from [NVMe AMD-Vi IO_PAGE_FAULT only with hardware IOMMU and fstrim/discard](https://bugzilla.kernel.org/show_bug.cgi?id=202665)

---

### 评论 #4 — syoyo (2019-05-15T06:53:45Z)

@seesturm Thanks! So the problem may be related to rock-dkms(kernel 4.15) or  GIGABYTE x399 mobo, not IOMMU.

Currenly my x399 mobo is in operation with NVIDIA GPUs so I couldn't test X399 + VEGAs + upstream kernel configuration soon. Will check it at some point(June or July 2019 planned) 
 

---

### 评论 #5 — jamilbk (2019-05-21T15:41:05Z)

@seesturm Yeah that may be it. I've tried `rocm-dkms` on kernels `4.15` and `4.18` and having IOMMU on causes the issue for me. As soon as the 2nd (or higher) GPU is used, it crashes.

I've added `amd_iommu=off` to my kernel boot line so all my GPUs are working for now.

@syoyo What BIOS version are you running? Perhaps this is an issue with Gigabyte boards?

---

### 评论 #6 — syoyo (2019-05-22T03:57:58Z)

@jamilbk you can find it in dmes log
...
### 评论 #7 — tasso (2023-12-11T14:39:18Z)

Has this issue been resolved?  If so, can we please close it? 

---

### 评论 #8 — syoyo (2023-12-11T14:55:01Z)

 Close the issue since its too old and cannot reprodce it.

---

### 评论 #9 — tasso (2023-12-11T17:02:56Z)
