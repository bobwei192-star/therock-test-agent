...
> **Issue #992**
> **状态**: closed
> **创建时间**: 2020-01-03T22:14:34Z
> **更新时间**: 2020-04-30T10:46:08Z
> **关闭时间**: 2020-04-30T10:46:08Z
> **作者**: athas
> **标签**: 
...
### 评论 #1 — kc2zgu (2020-03-17T14:10:27Z)

I've noticed this same problem. ROCm and Mesa both use LLVM, but ROCm uses its own modified version that conflicts with the system version Mesa is linked against when both are loaded into the same process. As far as I can tell this effectively makes virtually any GUI-based program (at least OpenGL-based ones like Blender) unusable with ROCm. I haven't been able to find any kind of official fix or workaround for this, or even whether it's considered a bug or if ROCm is just meant to only be used in applications that don't use OpenGL at the same time (which would be a serious limitation for most users).

---

### 评论 #2 — athas (2020-03-17T14:19:57Z)

That's also my diagnostic.  Interestingly, I have used ROCm 3.0 on NixOS 19.09, which has an older version of Mesa, where it works fine.

---

### 评论 #3 — athas (2020-03-17T15:02:25Z)

I can confirm that an otherwise-up-to-date NixOS, except with Mesa bumped down to 19.1.5 and compiled with LLVM 7, can have OpenGL and ROCm OpenCL co-exist in the same process.  Mesa 19.1.5 with LLVM 9 fails.  Next step for me is to try the newest NixOS Mesa (19.3.3) but with LLVM 7.

---

### 评论 #4 — athas (2020-03-18T11:46:50Z)

Mesa 19.3.3 with LLVM 8 seems to work as well.

---

### 评论 #5 — kc2zgu (2020-03-18T13:28:39Z)

So it theoretically can play nice with Mesa, just not if using LLVM 9? I have Mesa 20 with LLVM 9 (Gentoo) currently. I might see if I can build either that Mesa version or 19.3.5 against LLVM 8. The ebuild seems to depend on either 8 or 9, but I may have to modify it to be able to manually select which one to use without having to remove the llvm:9 slot.

---

### 评论 #6 — athas (2020-03-18T13:36:47Z)

Yes, in theory.  But there are so many moving parts here that it's hard to say whether what worked for me will also work for you.

---

### 评论 #7 — athas (2020-04-30T10:46:08Z)

Since this doesn't happen for me with the new versions of ROCm and Mesa, I'm closing the issue.
